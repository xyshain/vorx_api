<template>
    <div class="app-modal">
        ...
        <div class="card border-0 mb-4" style="box-shadow: 0 1rem 3rem rgba(0, 0, 0, 0.675) !important;">
            <div class="card-header background-blue">
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <span class="font-weight-bold h4 m-0 float-right text-white"> International</span>
                        <div class="clearfix"></div>
                        <span class="font-weight-bold h4 m-0 float-right text-white"> Enrolment Application Form</span>
                        <div class="clearfix"></div>
                        <span class="float-right text-white" style="font-size:12px;">RTO No: {{ app_settings.training_organisation_id }} | CRICOS Code: 03892A</span>
                    </div>
                </div>
            </div>
                ...
                <div>
                    ...
                    <div class="row px-4 pt-4" v-for="(v, key) in pages[cur_page].component" :key="key">
                        ...
                        <div v-for="(itm, k) in v.inputs" :key="k" v-bind:class="toType(itm['col_size']) !== 'undefined' ? 'col-md-'+itm['col_size'] : 'col-md-6'">
                            <div v-if="itm['type'] === 'label'">
                                <!-- text -->
                                <div class="form-group mt-3" :hidden="toType(itm['hidden']) !== 'undefined' ? itm['hidden'] : false">
                                    <h5 style="color: #858796 !important; font-weight: 700; font-size: 16px;">{{itm['name']}}</h5>
                                </div>
                            </div>
                            <div v-if="itm['type'] === 'text'">
                                <!-- text -->
                                <div class="form-group" :hidden="toType(itm['hidden']) !== 'undefined' ? itm['hidden'] : false">
                                 ...
                                 <div class="form-group">
                                    ...
                                    <div class="input-group-append">
                                        <button :class="'btn btn-'+app_color" type="button" @click="findCode(inputs[itm['name']] = itm['value'])">
                                            ...
                                            <span v-else-if="code_verified == true">Verified</span>
                                        </button>
                                    </div>
                                    </div>
                                    <div v-if="errors && errors['inputs.'+itm['name']]" class="badge badge-danger">{{ errors['inputs.'+itm['name']][0] }}</div>
                            ...
                            <div v-else-if="itm['type'] === 'multiselect'">
                                <!-- multiselect -->
                                <div class="form-group" v-if="itm['name'] != 'home_country_addr_suburb'" :hidden="toType(itm['hidden']) !== 'undefined' ? itm['hidden'] : false">
                                    <label v-bind:for="itm['name']">{{itm['label']}} <span v-if="toType(itm['required']) !== 'undefined'">*</span>
                                    <a
                                        v-if="toType(itm['tooltip']) !== 'undefined'"
                                        href="#"
                                        ...
                                        <table v-if="content.type == 'table'" width="100%" :class="'header-'+app_color">
                                            ...
                                            <tbody v-if="toType(content.body.tbody) !== 'undefined'">
                                                <tr v-for="(row, k_row) in content.body.tbody" :key="k_row">
                                                    ...
                                                    <!-- <td valign='top' v-for="(col, k_col) in row" :key="k_col" :class="'pt-2 align-top '+toType(content.body.text_type) !== 'undefined' ? content.body.text_type[k_col] : 'text-left'+ typeof content.body.background_color !== 'undefined' ?  ' '+content.body.background_color[k_col] : ''" v-html="col"></td> -->
...
export default {
    ...
    methods: {
        ...
        saveForm () {
            ...
            if (result.value) {
                ...
                axios.post('/international/online-enrolment/save',data)
                .then(function(res){
                    console.log(res);
                    if(res.data.status == 'success'){
                        window.location.href = '/international/online-enrolment/process/'+res.data.process;
                    }
                })
                .catch(function (err){
                    // console.log(err);
        ...
        findCode(data) {
            ...
            if(code !== " " && code !== null){
                ...
                axios({
                    method: 'get',
                    url: `/international/online-enrolment/agent/${code}`,
                }).then(response => {
    ...
    watch: {
        'inputs.currently_in_australia': function (newval, oldval) {
        let vm = this;
        let newPages = vm.pages;
        let newRequired = vm.required;
        
        newPages.forEach((element) => {
          element.component.forEach((elem) => {
            if (elem.title === "4. Contact Details") {
              elem.inputs.forEach((el) => {

                if (typeof el.hidden !== "undefined") {
                    console.log(el);
                  if (newval == false) {
                      if(el.name == 'Australian Residential Address'){
                        el.hidden = true;
                      }
                      if(el.name == 'addr_flat_unit_detail'){
                        el.hidden = true;
                      }
                      if(el.name == 'addr_building_property_name'){
                        el.hidden = true;
                      }
                      if(el.name == 'addr_street_num'){
                        el.hidden = true;
                      }
                      if(el.name == 'addre_street_name'){
                        el.hidden = true;
                      }
                      if(el.name == 'addr_suburb'){
                        el.hidden = true;
                      }
                      // if(el.hidden == true){
                      //     el.hidden = false;
                      ...
                      //     el.hidden = true;
                      // }
                      delete newRequired['addr_suburb'];
                      delete newRequired['addr_street_num'];
                      delete newRequired['addre_street_name'];
                      
                    //   if (el.name == 'home_country_res_addr' && typeof el.required !== "undefined") {
                    //       newRequired[el.name] = el.label;
                    //   }
                  } else {
                    if(el.name == 'Australian Residential Address'){
                        el.hidden = false;
                      ...
                      //   el.hidden = true;
                      // }
                    //   delete newRequired["home_country_res_addr"];
