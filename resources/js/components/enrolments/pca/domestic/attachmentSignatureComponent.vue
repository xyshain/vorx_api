            ...
            <div class="card-body p-0">
                <div>
                    ...
                    <div class="row px-4 pt-4">
                        <span class="font-weight-bold h5 m-0"> Attachment & Signature for <span :class="'text-'+this.app_color">{{student_name}}</span></span>
                    ...
                    <div class="row p-3">
                        ...
                        <div class="col-lg-6 col-md-6 col-sm-6 text-right">
                            ...
                            <button class="btn btn-success" @click="validateMandatoryDocs()" v-else><i class="far fa-save"></i> Finish Enrolment</button>
                        </div>
                    </div>
...
export default {
    ...
    created() {
        // console.log(window.mandatory_docs);
    ...
    methods: {
        back() {
            window.location.href = '/pca/online-enrolment/domestic/get/'+this.process_id;
        },
        undo() {
        ...
        saveForm() {
            ...
            if (result.value) {
                ...
                axios.post('/pca/online-enrolment/domestic/save-signature/'+process_id,{
                ...
                .then(function(res){
                    ...
                    if(res.data.status == 'success'){
                        // window.location.href = '/pca/online-enrolment/process/'+res.data.process;
                        swal.fire(
                            'Good job!',
                            'Signature Saved!',
                            'success'
        ...
        validateMandatoryDocs(){
            ...
            if(vm.attachments.length >= 3){
                    vm.mandatoryList.forEach(element => {
                        if(typeof vm.mandatory_docs[element.slug_name] === 'undefined' || vm.mandatory_docs[element.slug_name] == false){
                            //  console.log('yowo');
                            x = 0;
                            swal.fire(
                                'Oppss!',
        ...
        finishForm() {
            ...
            swal.fire({
            title: vm.user == true ? 'Save Changes?' : 'Finish Enrolment?',
            // text: "You won't be able to revert this!",
            type: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            ...
            if (result.value) {
                ...
                axios.post('/pca/online-enrolment/domestic/finish/'+process_id,{
                    process_id: vm.process_id,
                    ...
                    if(res.data.status == 'success'){
                        ...
                        // setTimeout(function() {
                            swal.fire({
                                ...
                                text: vm.user == true ? ' ' : "Your Enrolment form will be verified by our staff!",
                                type: "success",
...
<style scoped>
    ...
    ul.mandatory-docs {padding-left:0;}
    ul.mandatory-docs li{list-style: none;}
    ul.mandatory-docs li p{margin-bottom: 0;}
    ul.mandatory-docs li p span{position: relative;top: -2px;}
