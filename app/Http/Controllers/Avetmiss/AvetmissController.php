...
class AvetmissController extends Controller
{
    ...
    public function nat020($process_id)
    {
        ...
        foreach($nat20->get_data_avt20($this->studCom, $this->dateFrom) as $val) {
            // try {
    ...
    public function nat030($process_id)
    {
        ...
        foreach($nat30->get_data_avt30($this->studCom, $this->dateFrom) as $val) {
            // try {
            // if(!in_array($val->training_organisation_id, [null, '']) && !in_array($val->training_organisation_name, [null, ''])){
            // dump($val);
    ...
    public function nat080($process_id)
    {
        ...
        foreach($nat80->get_data_avt80($this->studCom) as $val) {
            ...
            if(in_array($val['student']['party']['person']['lastname'], [null,''])) {
                $nfe = $val['student']['party']['person']['firstname'] .', '. $val['student']['party']['person']['firstname'];
            }elseif(in_array($val['student']['party']['person']['firstname'], [null,''])) {
                $nfe = $val['student']['party']['person']['lastname'] .', '. $val['student']['party']['person']['lastname'];
            }else{
                $nfe = $val['student']['party']['person']['lastname'] ? $val['student']['party']['person']['lastname'].', ' : '';
                $nfe .= $val['student']['party']['person']['firstname'];
