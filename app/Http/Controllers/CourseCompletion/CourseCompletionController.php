    ...
    public function student_base($student)
    {
        ...
        foreach ($details->funded_course as $fundedCourse) {
            if($fundedCourse->offer_letter_course_detail_id == null){
                ...
                $checker = CompletionStudentCourse::where('student_course_id', $fundedCourse->offer_letter_course_detail_id)->get();
                if($checker->count() > 1){
                    $completion = CompletionStudentCourse::where('student_course_id', $fundedCourse->offer_letter_course_detail_id)->where('student_type',$fundedCourse->student->student_type_id)->first();
                }else{
            ...
            $course_detail = [
                'funded_course_id' => $fundedCourse->id,
        ...
        if ($request->course_completion == null) {
            try {
                ...
                foreach ($request->units as $key => $unit) {
                    array_push($statuses,$unit['status']);
                ...
                if(in_array(2,$statuses)){ //pag naay withdrawn na status
                    $student_course = FundedStudentCourse::find($request->funded_id);
                    $student_course->status_id = 7;
