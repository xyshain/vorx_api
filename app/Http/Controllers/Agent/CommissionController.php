...
class CommissionController extends Controller
{
    ...
    public function agent_commission_generate(AgentDetail $agent){
        $agent->load('commission_settings.sub_settings.student', 'commission_settings.cutoff_period', 'commission_settings.sub_settings.student_course', 'commission_settings.sub_settings.cutoff_period');
        $list = [];
            $student_count = 0;
            $name = $agent->user->party->name;
            $list['Company'] = $name;
            ...
            $com_setting = TrainingOrganisation::first();
                ...
                foreach ($cutoff->commission_details as $key => $commission_detail) {
                    ...
                    $student[$commission_detail->agent_commission_settings_sub_id] = [
                        ...
                        'course'        => $funded->course->code. ' - ' .$funded->course->name,
