...
class ReportsController extends Controller
{
    ...
    public function list_generator(Request $request)
    {
        ...
        foreach ($enrolments as $item){
            ...
            foreach ($item as $k => $v){
                ...
                $var = $letter.''.$number;
                // dump($var);
                if(in_array($k, ['Disability', 'Prior Education'])) {
                    $sheet->setCellValue($var, implode(', ',collect($v)->pluck('value')->toArray()));
                }else{
    ...
    public function student_list(Request $request)
    {
        ...
        if(!isset($request->automate) && \Auth::user()->hasRole('Demo')){
            ...
            $students = Student::where('student_type_id', $request->student_type == '*' ? '!=' : '=', $request->student_type)->where('is_test', 0)->get();
        }
            ...
            foreach($students as $stud){
                foreach($stud->completion as $com){
                    ...
                    if($funded_course){
                        ...
                        $enrolments[] = [
                            'student'=> $stud->student_id.' - '.$stud->party->name,
